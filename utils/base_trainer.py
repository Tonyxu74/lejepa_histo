...
class Trainer:
    ...
    def init_distributed(self):
        ...
        if self.args.world_size > 1:
            ...
            if self.args.local_rank is not None:
                ...
                assert torch.cuda.device_count() == self.args.world_size, 'set one GPU per process when using ' \
            ...
            # this env variable is defined when running with slurm
                ...
                assert torch.cuda.device_count() == 1, 'set one GPU per node in slurm for distributed training!'

            torch.cuda.set_device(self.args.device)
            print(f'CUDA device adjusted to device={self.args.device} for distributed training!')
            # initialize process group
            dist.init_process_group(
                backend='nccl', init_method='env://', world_size=self.args.world_size, rank=self.args.rank
            )
            print(f'Initialized (rank/world-size) {self.args.rank}/{self.args.world_size}')
        else:
            self.args.rank = 0
    ...
    def apply_ddp_to_model(self):
        ...
        """
        self.model = torch.nn.parallel.DistributedDataParallel(self.model, device_ids=[self.args.device])
    ...
    def get_datalists(self):
        ...
        if self.args.val_json == '':
            assert self.args.test_json == '', 'Validation set is required to choose best model when doing test!'
    ...
    def get_kfolds(self):
        ...
        assert self.args.num_folds == len(train_folds), 'train json does not define folds equal to num_folds'
        assert self.args.num_folds == len(val_folds), 'val json does not define folds equal to num_folds'
    ...
    def get_train_iterator(self, datalist):
        ...
        """
        train_ds = self.Dataset_Class(self.args, datalist, self.train_transforms, evaluate=False)
        if self.args.world_size > 1:
    ...
    def get_eval_iterator(self, datalist):
        ...
        """
        eval_ds = self.Dataset_Class(self.args, datalist, self.eval_transforms, evaluate=True)
        if self.args.world_size > 1:
            ...
            if self.args.rank == 0:
                ...
                for output in outputs:
                    ...
                    # no other model outputs should be accepted
                        ...
                        raise TypeError('Unexpected output type found when gathering outputs for distributed training!')
    ...
    def run(self):
        ...
        # run with kfolds
            ...
            if self.args.world_size > 1:
                raise ValueError('k-fold crossval not supported with distributed training!')
    ...
    def _run(self, fold=None):
        ...
        for epoch in range(self.args.num_epochs):
            ...
            if self.args.rank == 0:
                ...
                train_metrics, key_train_metric = self.compute_metrics(train_outputs, 'train')
                self.log_and_print_metrics(train_metrics, epoch)

                # if validating, check for new best model
                if self.do_val_test:
                    val_metrics, key_val_metric = self.compute_metrics(val_outputs, 'val')
                    ...
                    if key_val_metric > best_val_metric:
                        ...
                        print(f'new best model at epoch: {epoch} with val metric: {best_val_metric}')
        ...
        if self.do_val_test:
            ...
            print(f'testing with model saved on epoch {best_val_epoch} on rank {self.args.rank}')
            ...
            if self.args.rank == 0:
                test_metrics, key_test_metric = self.compute_metrics(test_outputs, 'test')
